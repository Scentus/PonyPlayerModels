-- Original file from CapsAdmin/pac3 on Github

local urltex = PPM.urltex or {}
...
concommand.Add("ppm_urltex_clear_cache", function()
	urltex.Cache = {}
	urltex.Queue = {}
end)
...
local enable = CreateClientConVar("ppm_enable_urltex", "1", true)
...
function urltex.GetMaterialFromURL(url, callback, skip_cache, shader, size, size_hack)
	...
	if type(callback) == "function" and not skip_cache and urltex.Cache[url] then
		...
		local mat = CreateMaterial("ppm_urltex_" .. util.CRC(url .. SysTime()), shader)
...
timer.Create("ppm_urltex_queue", 0.1, 0, urltex.Think)
...
function urltex.StartDownload(url, data)
	...
	local function start()
		...
		hook.Add("Think", id, function()
			...
			if go and time < timestamp then
				local vertex_mat = CreateMaterial("ppm_urltex_" .. util.CRC(url .. SysTime()), "VertexLitGeneric")
	...
	timer.Create(id, 5, 1, function()
		...
		if data.tries < 5 then
			print(string.format("material download %q timed out.. trying again for the %ith time", url, data.tries))
			-- try again
			data.tries = data.tries + 1
			urltex.GetMaterialFromURL(url, data)
			urltex.Queue[url] = data
		else
			print(string.format("material download %q timed out for good", url, data.tries))
		end
	end)
